...
class FileHandler:
    ...
    def clear_file(self) -> None:
        ...
        The file still remains open in the mode it was being used in.
        """
        im = self.file.mode
        self.open_file('w')
        self.file.write('')
    ...
    def make_copy(self, destination: str) -> "FileHandler":
        '''
        Copies the file to the specified destination.

        Returns the `FileHandler` for the destination file (which is closed).
        ...
        if not os.path.splitext(destination)[1]: # is directory
            hdl = FileHandler(
                filepath=f'{destination}/{self.filename}', 
    ...
    def read_file(self, read_mode: str | None = None) -> Any:
        ...
        If None, it reads in 'read(r+)' mode.
        '''
